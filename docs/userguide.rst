...
templates and scripts that Plone uses to build a page with different versions
...
Diazo themes, in contrast, are easy to understand for web designers and non-
developers alike.
...
1. One or more HTML mockups, also referred to as *theme* files, that represent
   ...
   usedesktop software like Dreamweaver or a text editor to create the relevant
...
3. A *rules file*, which defines how the placeholders in the theme (i.e. the
   ...
   Here, we are replacing the contents (child nodes) of placeholder element with
   HTML id ``main`` in the theme file (``theme.html``, found in the same
   directory as the ``rules.xml`` file, as referneced by the ``<theme />`` rule)
...
content mapes to the placeholders in this mockup.
...
files, so that it can be opened in a web browser from a local file. Plone will
convert these relative links to the appropriate absolute paths automatically,
ensuring the theme works no matter which URL the user is viewing.
...
You will also need a rules file called ``rules.xml``. If you haven't got one
...
Notice the declaration of the ``plone`` namespace at the top element. Place the
theme files and the ``rules.xml`` file into the ``theme`` directory.
...
children="..."`` or ``css:content-children="..."`` instead.
...
have e.g. both a ``css:theme`` and a ``theme`` attribute on a single rule.
...
``<rules />`` can be used with a `condition <#help_conditions>`_ to apply a
...
with the unconditional theme, if any, used as a fallback.
 ...
 ``<should be used.notheme />`` takes precedence overtheme />`` .
...
See the `Diazo documentation <http://diazo.org>`_ for more details about rules
that support ``if`` parameters and inline HTML and XSL.
